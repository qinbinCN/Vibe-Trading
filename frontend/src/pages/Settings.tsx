...
export function Settings() {
    ...
    Promise.all([api.getLLMSettings(), api.getDataSourceSettings()])
      ...
      .catch((error) => {
        ...
        if (isAuthRequiredError(error)) {
          ...
          toast.error(`${i18n.t("settings.llmSettingsLoadFailed")}: ${message}`);
          toast.error(`${i18n.t("settings.dataSourceSettingsLoadFailed")}: ${message}`);
        }
      })
      .finally(() => {
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      ...
      toast.error(`${i18n.t("settings.llmSettingsSaveFailed")}: ${error instanceof Error ? error.message : i18n.t("settings.unknownError")}`);
    } finally {
      setSaving(false);
    }
  };
  ...
  const submitDataSources = async (event: FormEvent) => {
    ...
    try {
      ...
      toast.error(`${i18n.t("settings.dataSourceSettingsSaveFailed")}: ${error instanceof Error ? error.message : i18n.t("settings.unknownError")}`);
    } finally {
      setDataSaving(false);
    }
  };
    ...
    <form onSubmit={submitLocalApiKey} className="rounded-lg border bg-card p-5 shadow-sm">
      ...
      <div className="grid gap-3 md:grid-cols-[minmax(0,1fr)_auto]">
        <label className="grid gap-2">
          <span className={labelClass}>{i18n.t("settings.localApiKey")}</span>
          <input
            type="password"
            value={localApiKey}
            onChange={(event) => setLocalApiKeyState(event.target.value)}
            className={fieldClass}
            placeholder={i18n.t("settings.localApiKeyHint")}
      ...
      <p className="mt-2 text-xs text-muted-foreground">{i18n.t("settings.localApiKeyHint")}</p>
    </form>
  );
    ...
    return (
      <div className="mx-auto max-w-5xl space-y-6 p-6">
        <div className="space-y-2">
          ...
          <p className="max-w-3xl text-sm text-muted-foreground">{i18n.t("settings.pageDesc")}</p>
        </div>
        ...
        <div className="flex min-h-32 items-center justify-center rounded-lg border bg-card p-5 text-sm text-muted-foreground">
          {settingsLoadError ? (
            <div className="text-center">
              <div className="font-medium text-foreground">{i18n.t("settings.settingsUnavailable")}</div>
  ...
  const keyStatus = settings.api_key_configured
    ? i18n.t("settings.llmApiKeyConfigured")
    : settings.api_key_required
      ? i18n.t("settings.llmApiKeyPlaceholder")
      : selectedProvider?.auth_type === "oauth" && selectedProvider.login_command
        ? `${i18n.t("settings.llmOauthRequired")} ${selectedProvider.login_command}`
        : i18n.t("settings.llmNoApiKeyRequired");
  ...
  const tushareStatus = dataSettings.tushare_token_configured
    ? i18n.t("settings.llmApiKeyConfigured")
    : i18n.t("settings.tushareTokenPlaceholder");
      ...
      <form onSubmit={submit} className="grid gap-6 lg:grid-cols-[minmax(0,1.4fr)_minmax(320px,0.8fr)]">
        <section className="rounded-lg border bg-card p-5 shadow-sm">
          <div className="mb-5 flex items-center gap-2">
            ...
            <h2 className="text-base font-semibold">{i18n.t("settings.llmConnection")}</h2>
          </div>
          ...
          <div className="grid gap-4">
            <label className="grid gap-2">
              ...
              <span className={hintClass}>{i18n.t("settings.llmProviderHint")}</span>
            </label>

            <label className="grid gap-2">
              <span className={labelClass}>{i18n.t("settings.modelName")}</span>
              <div className="flex gap-2">
                ...
                <button
                  ...
                  title={i18n.t("settings.llmUseProviderDefaults")}
                >
                  <RotateCcw className="h-4 w-4" />
                  <span className="hidden sm:inline">{i18n.t("settings.llmUseProviderDefaults")}</span>
                </button>
              </div>
              <span className={hintClass}>{i18n.t("settings.llmModelHint")}</span>
            </label>

            <label className="grid gap-2">
              <span className={labelClass}>
                {selectedProvider?.auth_type === "oauth" ? "OAuth" : i18n.t("settings.llmApiKey")}
              </span>
              ...
              <div className="flex items-center justify-between gap-3">
                ...
                {selectedProvider?.api_key_required ? (
                  <label className="flex shrink-0 items-center gap-2 text-xs text-muted-foreground">
                    ...
                    />
                    {i18n.t("settings.llmClearApiKey")}
                  </label>
                ) : null}
              </div>
            </label>
        ...
        <section className="rounded-lg border bg-card p-5 shadow-sm">
          <div className="mb-5 flex items-center gap-2">
            ...
            <h2 className="text-base font-semibold">{i18n.t("settings.llmGeneration")}</h2>
          </div>
          ...
          <div className="grid gap-4">
            ...
            <label className="grid gap-2">
              ...
              <select
                ...
                className={fieldClass}
              >
                <option value="">{i18n.t("settings.reasoningEffortOff")}</option>
                <option value="low">low</option>
                <option value="medium">medium</option>
                <option value="high">high</option>
                <option value="max">max</option>
              </select>
              <span className={hintClass}>{i18n.t("settings.reasoningEffortHint")}</span>
            </label>

            <div className="rounded-md border bg-muted/30 px-3 py-2 text-xs text-muted-foreground">
              <span className="font-medium text-foreground">{i18n.t("settings.llmEnvPath")}: </span>
        ...
        <div className="grid gap-5 lg:grid-cols-[minmax(0,1.1fr)_minmax(280px,0.9fr)]">
          <div className="grid gap-4">
            <label className="grid gap-2">
              ...
              <div className="flex items-center justify-between gap-3">
                <span className={hintClass}>{i18n.t("settings.tushareTokenHint")}</span>
                <label className="flex shrink-0 items-center gap-2 text-xs text-muted-foreground">
                  <input
                    type="checkbox"
          ...
          <div className="rounded-md border bg-muted/20 p-4">
            <div className="mb-3 flex items-center justify-between gap-3">
              <span className="text-sm font-medium">{i18n.t("settings.baostockStatus")}</span>
              <span className={`rounded-full px-2 py-0.5 text-xs ${dataSettings.baostock_supported ? "bg-success/10 text-success" : "bg-warning/10 text-warning"}`}>
                {dataSettings.baostock_supported ? i18n.t("settings.baostockSupported") : i18n.t("settings.baostockNotSupported")}
              </span>
            </div>
            <div className="space-y-2 text-sm text-muted-foreground">
              ...
              <p>
                {dataSettings.baostock_installed
                  ? i18n.t("settings.baostockPackageInstalled")
                  : i18n.t("settings.baostockPackageMissing")}
              </p>
            </div>
          </div>
        </div>
