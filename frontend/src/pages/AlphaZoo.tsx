...
const ZOO_CARDS: ZooCard[] = [
  ...
  },
  {
    ...
    descriptionKey: "cardAcademic",
    approxCount: 6,
    accent: "from-violet-500/20 to-violet-500/5",
  },
];

function getUniverseOptions() {
  return [
    { value: "csi300", label: i18n.t("alphaZoo.universeCsi300") },
    { value: "sp500", label: i18n.t("alphaZoo.universeSp500") },
    { value: "btc-usdt", label: i18n.t("alphaZoo.universeBtc") },
  ];
}
...
function BrowseView() {
  ...
  useEffect(() => {
    ...
    api
      ...
      .catch((err: unknown) => {
        if (!alive) return;
        const msg = err instanceof Error ? err.message : i18n.t("alphaZoo.failureToLoadAlphas");
        toast.error(msg);
        setAlphas([]);
        setTotal(0);
      })
    ...
    <div className="p-4 md:p-8 max-w-6xl mx-auto space-y-8">
          ...
          return (
            ...
            >
              ...
              <p className="text-xs text-muted-foreground line-clamp-3">
                {i18n.t("alphaZoo." + z.descriptionKey)}
              </p>
            </button>
          );
        })}
      ...
      <div className="flex flex-col md:flex-row md:items-end gap-3 border rounded-xl p-4 bg-card">
        ...
        <Link
          ...
          title={i18n.t("alphaZoo.tickGuidance")}
        >
          ...
          <table className="w-full text-sm" aria-label={i18n.t("alphaZoo.alphaCatalogue")}>
            ...
            <thead>
              <tr className="border-b bg-muted/40">
                ...
                <th className="text-right px-4 py-2.5 text-muted-foreground" title={i18n.t("alphaZoo.decayTitle")}>
                  ...
                  >
                    <td className="px-3 py-2">
                      <input
                        ...
                        aria-label={i18n.t("alphaZoo.selectForCompareAria", { id: a.id })}
...
function DetailView({ alphaId }: DetailProps) {
  ...
  useEffect(() => {
    ...
    api
      ...
      .catch((err: unknown) => {
        if (!alive) return;
        const msg = err instanceof Error ? err.message : i18n.t("alphaZoo.couldNotLoad");
        setError(msg);
      })
      .finally(() => {
        if (alive) setLoading(false);
  ...
  if (loading) {
    return (
      <div className="p-8 flex items-center justify-center text-muted-foreground">
        <Loader2 className="h-4 w-4 animate-spin mr-2" aria-hidden="true" /> {i18n.t("alphaZoo.loadingDetail", { id: alphaId })}
      </div>
    );
  }
      ...
      <section className="space-y-2">
        ...
        <div className="border rounded-xl overflow-hidden">
          <table className="w-full text-sm">
            <tbody>
              <MetaRow label={i18n.t("alphaZoo.themeKey")} value={metaString(meta, "theme")} />
...
function BenchView() {
  ...
  const startBench = async (e: FormEvent) => {
    ...
    try {
      ...
      const msg = err instanceof Error ? err.message : i18n.t("alphaZoo.failureToStartBench");
      ...
      if (msg.toLowerCase().includes("single-asset")) {
        setFormError(
          `${msg} ${i18n.t("alphaZoo.singleAssetHint")}`,
        );
      } else {
        toast.error(msg);
      }
  ...
  const attachStream = (newJobId: string) => {
    ...
    source.addEventListener("error", (e) => {
      ...
      let msg = i18n.t("alphaZoo.benchStreamError");
      try {
    ...
    <div className="border rounded-xl p-4 bg-card space-y-3">
      <div className="flex items-center justify-between text-xs text-muted-foreground">
        <span className="flex items-center gap-1.5">
          ...
          {jobId ? i18n.t("alphaZoo.jobLabel", { id: jobId.slice(0, 12) }) : i18n.t("alphaZoo.submitting")}
        </span>
        {progress && (
...
function ResultPanel({ result }: { result: AlphaBenchResult }) {
  ...
  useEffect(() => {
    ...
    const aliveLabel = i18n.t("alphaZoo.alive");
    const reversedLabel = i18n.t("alphaZoo.reversed");
    const deadLabel = i18n.t("alphaZoo.dead");

    chart.setOption({
      backgroundColor: "transparent",
      tooltip: { trigger: "axis", axisPointer: { type: "shadow" } },
      legend: {
        data: [aliveLabel, reversedLabel, deadLabel],
      ...
      series: [
        { name: aliveLabel, type: "bar", stack: "n", data: aliveSeries, itemStyle: { color: theme.upColor } },
        { name: reversedLabel, type: "bar", stack: "n", data: reversedSeries, itemStyle: { color: theme.warningColor } },
        { name: deadLabel, type: "bar", stack: "n", data: deadSeries, itemStyle: { color: theme.downColor } },
      ],
    });
...
function getSortOptions() {
  return [
    { value: "ir", label: i18n.t("alphaZoo.sortIr") },
    { value: "ic_mean", label: i18n.t("alphaZoo.sortIcMean") },
    { value: "ic_positive_ratio", label: i18n.t("alphaZoo.sortIcPositive") },
    { value: "ic_count", label: i18n.t("alphaZoo.sortSampleCount") },
  ];
}
...
function CompareView() {
  ...
  const attachStream = (newJobId: string) => {
    ...
    source.addEventListener("error", (e) => {
      ...
      let msg = i18n.t("alphaZoo.compareStreamError");
      try {
        const data = JSON.parse((e as MessageEvent).data || "{}");
        if (typeof data.message === "string") msg = data.message;
      } catch {
  ...
  const startCompare = async (e: FormEvent) => {
    ...
    try {
      ...
      const msg =
        err instanceof Error ? err.message : i18n.t("alphaZoo.failureToStartCompare");
      toast.error(msg);
      setStatus("error");
    }
  };
      ...
      <form onSubmit={startCompare} className="border rounded-xl p-4 bg-card space-y-3">
        <div>
          <label htmlFor="compare-ids" className="text-xs text-muted-foreground block mb-1">
            {i18n.t("alphaZoo.alphaIds")}{ids.length > 0 ? ` (${i18n.t("alphaZoo.selected", { count: ids.length })})` : ""}
          </label>
          <textarea
            id="compare-ids"
            value={idsText}
            ...
            disabled={busy}
            rows={2}
            placeholder={i18n.t("alphaZoo.alphaIdsPlaceholder")}
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-sm">
        ...
        <span className="text-muted-foreground">
          {result.n_compared} {i18n.t("alphaZoo.compared")} · {i18n.t("alphaZoo.rankedBy")} {result.sort} · {result.universe} · {result.period}
        </span>
        {result.n_skipped > 0 && (
          <span className="inline-flex items-center gap-1 text-amber-600 dark:text-amber-400">
            <AlertTriangle className="h-3.5 w-3.5" aria-hidden="true" /> {result.n_skipped} {i18n.t("alphaZoo.skipped")}
          </span>
        )}
      </div>
          ...
          <table className="w-full text-sm" aria-label={i18n.t("alphaZoo.alphaCatalogue")}>
            <thead>
              <tr className="border-b bg-muted/40 text-muted-foreground text-xs">
                <th className="text-right px-3 py-2">{i18n.t("alphaZoo.rank")}</th>
                ...
                <th className="text-right px-3 py-2 hidden lg:table-cell" title={i18n.t("alphaZoo.sampleCountTitle")}>{i18n.t("alphaZoo.sampleCount")}</th>
                <th className="text-right px-3 py-2" title={i18n.t("alphaZoo.deltaSortTitle", { sort: result.sort })}>{i18n.t("alphaZoo.deltaSort", { sort: result.sort })}</th>
              </tr>
            </thead>
            <tbody>
      ...
      {result.skipped.length > 0 && (
        <p className="text-xs text-muted-foreground">
          <span className="font-medium">{i18n.t("alphaZoo.skippedLabel")}</span>{" "}
