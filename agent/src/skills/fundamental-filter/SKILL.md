...
2. **`get_market_data(codes=["600519.SH"], fields=["close"], max_rows=10)`** — returns
   the last 10 closing prices (shows recent trend, compact output).  Use
   `fields=["close","volume","turnover_rate"]` for richer context.  When
   `TDX_ROOT_PATH` is set, reads local .day files.
   **Always use max_rows=10 when you only need current price + recent trend.**
