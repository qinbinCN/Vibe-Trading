...
6. **数据来源**：A股财报优先使用 `get_financial_snapshot` 工具（本地通达信 gpcw*.zip，576字段中文命名，零网络）。当 `TDX_ROOT_PATH` 未配置时才回退到 tushare 在线 API。PE/PB/ROE等估值指标需配合 `get_market_data` 获取的当前股价计算。
